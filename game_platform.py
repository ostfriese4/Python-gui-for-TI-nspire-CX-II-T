  ...
  class platform:
    ...
    def init(self,app):
      if self.user1 is None or self.user2 is None:
        ...
        l=widgets.Label(w,text="Bitte gebt eure Namen ein:",x=0,y=0,width=100,height=0.25)
        ...
        b=widgets.Button(w,text="Weiter",x=0,y=0.75,width=1,height=0.25,text_align="center")
        def ready():
          self.user1=user(n1.get_text())
          self.user2=user(n2.get_text())
          w.close()
          self.resetApp=k.get("apps")(name="Spiele zurücksetzen",categories=["Spiele"],startcode=self.reset)
          app()
